...
sub process_failed_items {
    ...
    my $error_count;
    # If $maxcount is actually larger than the list's size, it doesn't matter. Redis is quite
    #   tolerant there, and just takes whatever in the list and that's it.
    my @item_keys = $rh->lrange($temp_table, -$max_count, -1);
    my $item_count = $max_count || $failed_items;
