...
sub report {
    ...
    $unprocessed_q and is(
        $q->queue_length('unprocessed'),
        $unprocessed,
        "$unprocessed elements are currently in the queue."
    );
    $working_q and is(
        $q->queue_length('working'),
        $in_progress,
...
sub test_reliable_fifo {
    ...
    $element0 = $q->enqueue_items({ items => [ 3 ] })->[0];
    $unprocessed++;
    report('After enqueuing a third item', [ 1, 2, 3 ], []);

    # Illegally :-D simulating failure...
    $q->redis_handle->rpoplpush($q->_unprocessed_queue, $q->_failed_queue)
        for 1 .. 3;
    $unprocessed -= 3; $failed += 3;
    report('After simulating failure and evacuating the unprocessed queue', [], [], [ 1, 2, 3 ]);

    $items_in = $q->handle_failed_items({ action => 'requeue' });
    $unprocessed += 3; $failed -= 3;
    report('After requeuing all the failed items to the unprocessed queue', [ 1, 2, 3 ], [], []);
    ...
    $q->enqueue_items({ items => [ 1, 2, 3, 4, 5, 6, 7, 8 ] })->[0];
    $unprocessed += 5;
    report('After enqueuing 5 items in one go', [ 1, 2, 3, 4, 5, 6, 7, 8 ], []);

    $q->redis_handle->rpoplpush($q->_unprocessed_queue, $q->_failed_queue)
        for 1 .. 6;
    $unprocessed -= 6; $failed += 6;
    report('After simulating failure for 6 items out of 8', [ 7, 8 ], [], [ 1, 2, 3, 4, 5, 6 ]);

    my @failed = ();
    $q->process_failed_items({ max_count => 3, callback => sub { push @failed, shift->data });
    report('After process_failed_items()', [ 7, 8 ], [], [ 4, 5, 6 ]);
    is_deeply(\@failed, [ 1, 2, 3 ], 'process_failed_items() works in the correct order.');
    ...
    $unprocessed -= 3;
    $in_progress += 3;
    report('After evacuating the unprocessed queue', [], [ 1, 2, 3 ]);

    $q->mark_items_as_processed({ items => [ $element2, $element1, $element0 ] });
    $processed += 3;
    $in_progress -= 3;
    report('After evacuating the working queue (out of order to test that as well)', [], []);
